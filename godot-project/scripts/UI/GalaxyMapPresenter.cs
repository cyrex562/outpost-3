...
/// </summary>
public partial class GalaxyMapPresenter : Control
{
	...
	// Time control (using shared TimeManager for galaxy context)
	...
	private GalaxyMapSystemDetailsModalPresenter _systemDetailsModal = null!;  // Initialized in _Ready() by loading scene
	...
	public override void _Ready()
	{
		...
		// Initialize TimeManager with Galaxy context for long-term probe travel
		_timeManager = new TimeManager(_stateStore, TimeScaleContext.Galaxy);
		...
		_systemDetailsModal = modalInstance as GalaxyMapSystemDetailsModalPresenter
	...
	public override void _Process(double delta)
	{
		...
		// Handle time advancement using shared TimeManager with Galaxy context
		if (_timeManager != null)
		{
			var gameState = _stateStore.State;
			_timeManager.Update(delta, gameState.CurrentSpeed, gameState.IsPaused);
		}
	}
	...
	private void UpdateSelectionInfo()
	{
		...
		// Show estimated probe travel time (at 0.9c speed)
		if (system.DiscoveryLevel != DiscoveryLevel.Explored)
		{
			var travelTime = PhysicsConstants.FormatProbeTraverTime(system.DistanceFromSol);
			_systemInfoLabel.Text += $" | Probe ETA: {travelTime}";
		}
	...
	private void OnLaunchProbePressed()
	{
		...
		if (_stateStore.DebugSettings.DebugActions)
		{
			...
			var travelTime = PhysicsConstants.FormatProbeTraverTime(_selectedStar.System.DistanceFromSol);
			GD.Print($"  ✓ Estimated travel time: {travelTime} (at 0.9c)");
	...
	private void OnPausePressed()
	{
		...
		// Use shared game state command instead of local fields
	...
	private void OnSlowDownPressed()
	{
		...
		var currentSpeed = (int)currentState.CurrentSpeed;
	...
	private void UpdateTimeControlUI()
	{
		...
		{
			// Show contextual speed labels for galaxy map
			var speedText = DisplayFormatter.FormatGameSpeedWithContext(currentState.CurrentSpeed, isGalaxyContext: true);
			_timeSpeedLabel.Text = $"Time: {speedText}";
		}
...
public partial class StarNode : Node2D
{
	...
	public override void _Draw()
	{
		...
		if (shouldShowLabel)
		{
			...
			var ly = Position / GalaxyMapPresenter.BASE_PIXELS_PER_LY;
...
public partial class ProbeNode : Node2D
{
	...
	public void Initialize(ProbeInFlight probe, StarSystem targetSystem, double currentGameTime)
	{
		...
		_targetWorldPos = targetLYPos * GalaxyMapPresenter.BASE_PIXELS_PER_LY;
