# GEMINI.md - Google Gemini Guide for Outpost 3
...
**Key Components:**
- **Events** (`scripts/Core/Events/`): Immutable records of state changes
...
- **Commands** (`scripts/Core/Commands/`): Encapsulated actions that generate events
  - `ICommand`: Command interface
  - `InitializeGalaxy`, `LaunchProbe`, `AdvanceTime`, etc.
  - Commands validate input and produce events

- **Domain Models** (`scripts/Core/Domain/`): Core game entities
...
- **GameServices** (`Autoload/GameServices.cs`): Central service container auto-loaded at startup
...
6. **Document** any new mechanics in `docs/`
...
2. Create C# script in `scripts/UI/`
...
5. Check `DebugSettings.cs` for debug options

### When Generating Code:
1. **Follow C# naming conventions**: PascalCase for classes, methods, properties; camelCase for private fields
2. **Use nullable reference types** appropriately in C#
3. **Leverage C# features**: LINQ, pattern matching, async/await where appropriate
4. **Keep Godot lifecycle in mind**: `_Ready()`, `_Process()`, `_PhysicsProcess()`
5. **Use Godot signals** for event-driven UI updates
...
4. Avoid deep nesting - extract helper methods
...
- When analyzing code, consider both the C# and Godot contexts
- Leverage your multimodal capabilities to understand scene files and UI layouts
- When suggesting code changes, provide complete, compilable examples
- Consider performance implications for game loop code
- Think about how changes affect the event sourcing and replay capabilities

## Questions or Issues?
Refer to existing documentation in `docs/` or review test examples in `Tests/` for patterns and conventions.
