...
## Phase 3.5: Desktop/WASM UI Pivot

**Goal**: Deliver a native desktop UI (Windows/Linux) with the same codebase compiled to WASM. Provide a 2D hex map, panels/modals, and on-demand charts without webviews.

**References**:
- [docs/UI_REDESIGN_PLAN.md](UI_REDESIGN_PLAN.md) (historical web plan)
- [docs/desktop_wasm_pivot.md](../desktop_wasm_pivot.md) (this pivot’s detailed plan)

**Design Inspiration**: Outpost (1994), Europa Universalis IV/V, SimCity

### Technology Decision
- [x] Evaluate Web App vs WASM Desktop approaches
- [x] Decision: Bevy + bevy_egui (immediate-mode UI) for desktop and WASM
- [x] No webviews; charts via `egui_plot`

### Phase 3 Completion (Prerequisites)
- [ ] Building upgrade system UI (port/skeleton)
- [ ] Building detail modal/panel (egui)
- [ ] Power status per building indicators
- [ ] Production output indicators

### Stage 1: Workspace & Architecture
- [ ] Create `outpost-core` crate (domain, commands, events, simulation)
- [ ] Define stable `core` API for queries and commands
- [ ] Create `outpost-client` crate (Bevy + bevy_egui front-end)
- [ ] Introduce `Storage` trait; desktop and WASM backends
- [ ] Feature flags for `desktop` and `wasm`

### Stage 2: Hex Map Foundation
- [ ] Implement axial hex coordinates (q,r) and math helpers
- [ ] Camera pan/zoom with bounds and sensible zoom limits
- [ ] Render hex grid and basic terrain tiles
- [ ] Selection highlight and hover tooltips
- [ ] Placement workflow: ghost preview + confirm to place building
- [ ] Toggleable data layers: power, resources, pollution (skeleton)
- [ ] Network visualization (rail lines, wormhole connections) placeholder

### Stage 3: UI Panels & Modals (egui)
- [ ] Sidebar/outliner (buildings list, alerts)
- [ ] Building Detail modal (stats, production, actions)
- [ ] Construction modal (types, costs, placement)
- [ ] Bottom bar: notifications, turn controls; top resource bar
- [ ] Keyboard shortcuts: Space (turn), B (build), Esc (close)

### Stage 4: Charts (egui_plot)
- [ ] Power chart (generation vs consumption)
- [ ] Resources chart (stock levels, prod vs cons)
- [ ] Population chart (total, employed/unemployed)
- [ ] On-demand update (regenerate series on view open/click)

### Stage 5: Persistence
- [ ] Desktop saves to disk (RON/JSON) with autosave every N turns
- [ ] WASM saves via IndexedDB; basic profile management

### Stage 6: CI & Artifacts
- [ ] GitHub Actions: matrix build for Windows/Linux
- [ ] WASM build pipeline (wasm-bindgen/trunk) with static host page
- [ ] Upload native binaries and WASM bundle as artifacts

### Stage 7: Assets & Polish
- [ ] Initial sprites for terrain/resources/buildings
- [ ] Performance pass (sprite batching, culling)
- [ ] Input normalization between desktop and WASM

### Testing
- [ ] Unit tests for hex math and selection
- [ ] Unit tests for core simulation invariants
- [ ] Smoke test for client startup (headless where possible)
- [ ] WASM run smoke in CI (basic load + init)
