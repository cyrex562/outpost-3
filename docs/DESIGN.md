...
**What's implemented now.** The generic mechanism — `BuildingDef.storage`, `colony::storage_capacities`, `ColonyResourcePool::bank_and_clear` wired into the turn pipeline in place of the unconditional `clear()` — plus two worked examples: `battery_bank` (`content/base/buildings.yaml`), a passive storage building banking 40 MW-equivalent of `power`, and the full water migration described above (`water_tank` storage, `water_bottling_plant` crating). Proven by `outpost_core`'s `power_surplus_evaporates_every_sol_without_a_storage_building` (regression: unchanged pre-#348 behaviour with no storage building present) and `battery_bank_carries_surplus_power_across_sols_up_to_its_capacity` (surplus survives into the next sol up to capacity, and still evaporates above it), plus the water-specific tests listed above.

**What's filed as follow-up issues, per this section's own decisions above:** waste as content (the resource itself, `waste_processing` capacity, and bunker storage); hex contamination + tech-gated remediation; networking/compute as a utility feeding population morale; the maintenance condition-stat-plus-breakdown-roll failure model; fixed-link capacity/loss on infrastructure edges.
