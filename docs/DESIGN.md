...
**What's implemented now.** The generic mechanism — `BuildingDef.storage`, `colony::storage_capacities`, `ColonyResourcePool::bank_and_clear` wired into the turn pipeline in place of the unconditional `clear()` — plus three worked examples: `battery_bank` (`content/base/buildings.yaml`), a passive storage building banking 40 MW-equivalent of `power`, the full water migration described above (`water_tank` storage, `water_bottling_plant` crating), and — issue #386 — `waste` itself: a per-sol flow resource produced as a byproduct of refining recipes (`refine_ore_to_plate`, `smelt_conductive_ore` each now output 1 kg of `waste` alongside their main product), banked by a `waste_bunker` (40 kg capacity, same shape as `battery_bank`/`water_tank`, no recipe of its own), plus a `waste_processing` capacity resource re-established each sol by a `recycling_plant`'s `process_waste` recipe — authored identically to `housing`, a standing capacity rather than a stock. Note that #386 introduces the resource, its byproduct production, and its storage/capacity content only; nothing yet compares `waste` production against `waste_processing` capacity plus `waste_bunker` headroom, and no hex-contamination consequence exists — that comparison and consequence is #387's scope, and crating waste into a shippable commodity is #389's. Proven by `outpost_core`'s `power_surplus_evaporates_every_sol_without_a_storage_building` (regression: unchanged pre-#348 behaviour with no storage building present) and `battery_bank_carries_surplus_power_across_sols_up_to_its_capacity` (surplus survives into the next sol up to capacity, and still evaporates above it), the water-specific tests listed above, and — for #386 — `outpost_web`'s `refinery_recipes_emit_waste_byproduct_from_real_pack`, `waste_bunker_carries_waste_across_sols_up_to_its_capacity`, and `recycling_plant_reestablishes_waste_processing_capacity_each_sol`.

**What's filed as follow-up issues, per this section's own decisions above:** crating waste into a shippable commodity (#389); networking/compute as a utility feeding population morale (#382); the maintenance condition-stat-plus-breakdown-roll failure model (#384); fixed-link capacity/loss on infrastructure edges (#383).
