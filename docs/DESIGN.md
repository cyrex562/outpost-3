...
**Water becomes a utility, superseding §5's earlier decision.** Water was first classified as a commodity on the rule "utilities are unstockable, stocks are commodities" — reasonable when nothing made power stockable either. Once storage is a building rather than a property of the substance, that rule collapses: power is the archetypal utility and it turns out to be stockable too, given a battery. There is no principled reason left for water to be the one utility still modelled as cargo, so it moves to `ColonyResourcePool`/`resources.yaml` alongside power, with the same tank/pipeline/crating treatment. **This migration is not done in this pass** — water still lives in `ColonyPool`/`commodities.yaml` as of this section landing (see "What's implemented now" below) — but the destination is decided, and a colony's need-reserve/trade-surplus logic (§5's "Only surpluses of survival commodities are tradeable") will need to move with it.
...
**What's implemented now.** The generic mechanism — `BuildingDef.storage`, `colony::storage_capacities`, `ColonyResourcePool::bank_and_clear` wired into the turn pipeline in place of the unconditional `clear()` — plus one worked example: `battery_bank` (`content/base/buildings.yaml`), a passive storage building banking 40 MW-equivalent of `power`. Proven by `outpost_core`'s `power_surplus_evaporates_every_sol_without_a_storage_building` (regression: unchanged pre-#348 behaviour with no storage building present) and `battery_bank_carries_surplus_power_across_sols_up_to_its_capacity` (surplus survives into the next sol up to capacity, and still evaporates above it).

**What's filed as follow-up issues, per this section's own decisions above:** migrating water from `ColonyPool`/commodity to `ColonyResourcePool`/utility; waste as content (the resource itself, `waste_processing` capacity, and bunker storage); hex contamination + tech-gated remediation; networking/compute as a utility feeding population morale; the maintenance condition-stat-plus-breakdown-roll failure model; fixed-link capacity/loss on infrastructure edges; and crating recipes once a storable utility other than power exists to crate.
