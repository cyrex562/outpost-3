...
Fast-forward itself was already implemented — `GameEngine::advance_until_interrupted` plus `collect_turn_interrupts`, with `interrupt::Tier` (`Urgent` halts, `Notable` digests) and `ui::TimeControlState`/`InterruptDigestData` as the surrounding contract. What it lacks is exposure: no host command or route reaches it, and no UI drives it. That is the remaining work on #332.
