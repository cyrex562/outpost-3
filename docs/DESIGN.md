...
**Waste overflow: hex contamination.** When waste outruns processing and storage, it contaminates the colony's hex: habitability drops and deposit yields fall. Chosen over a stability hit, back-pressure on producers, or population health effects, because it is the only genuinely *new* kind of consequence — the others reuse levers brownout and labour shortage already pull, or land on stability, an increasingly undifferentiated "things are bad" number. It also makes the 100 km hex map (§8.1) matter beyond site selection: neglect can foul the ground a colony lives on. **Contamination is remediable** — the right technology lets a player clean up a contaminated hex and remove the penalty, so it is a lasting consequence rather than a permanent one. Needs per-hex contamination state, a tech-gated cleanup action, and UI showing affected hexes and severity. Not implemented in this pass; filed as a follow-up (see below).
...
**What's filed as follow-up issues, per this section's own decisions above:** hex contamination + tech-gated remediation (#387, #388); crating waste into a shippable commodity (#389); networking/compute as a utility feeding population morale (#382); the maintenance condition-stat-plus-breakdown-roll failure model (#384); fixed-link capacity/loss on infrastructure edges (#383).
