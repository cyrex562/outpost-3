...
### 5A. Utilities: storage is a building, not a property of the substance (issue #348)

**The core model.** Every utility — power, water, atmosphere, waste, housing, networking/compute, maintenance — is a per-sol flow by default: produced and consumed in the same sol, surplus lost. **Storage is an opt-in building**: a battery banks power, a tank banks water, a bunker banks waste. Building storage is what converts a flow into something that carries across sols, up to the capacity built. **Transport is opt-in infrastructure**, either a fixed link (powerline, pipeline, network link) or crating the utility into a shippable commodity for convoy. Power, water, and waste therefore all work *identically*, differing only in which storage and transport buildings exist for them — "why is water cargo but atmosphere isn't" stops being a question, because nothing is intrinsically cargo.

| Resource | Utility? | Storage | Transport |
|---|---|---|---|
...
| networking / compute | yes | — | network link |
| maintenance | yes | — | none |
| research | no | n/a | none |
| food | no | commodity pool | convoy |
| fuel | no | commodity pool | convoy or pipeline |

**Water becomes a utility, superseding §5's earlier decision — migrated (issue #380).** Water was first classified as a commodity on the rule "utilities are unstockable, stocks are commodities" — reasonable when nothing made power stockable either. Once storage is a building rather than a property of the substance, that rule collapses: power is the archetypal utility and it turns out to be stockable too, given a battery. There is no principled reason left for water to be the one utility still modelled as cargo, so it now lives in `ColonyResourcePool`/`resources.yaml` alongside power. Reclassifying an id is purely declarative — `ColonyStores`' dispatch (`ContentRegistry::is_resource`) and every needs/production code path that reads it were already generic, so moving `water`'s definition from `commodities.yaml` to `resources.yaml` was sufficient on its own to route it correctly everywhere, with zero changes to `needs.rs`/`production.rs`/`stores.rs`.

Two real consequences fell out of the move, both deliberate, not incidental:

- **Water is no longer directly tradeable.** `run_trade_flow`'s tradeable-id set is built solely from `Colony.pool.commodity_ids()` (§5's original structural guarantee), so a resource-classified id is invisible to trade the same way `power`/`housing` already were — no code change needed to *remove* it, the same way none was needed to add it in #304. §5's "Only surpluses of survival commodities are tradeable" reserve logic is generic over `NeedsConfig`, not water-specific, so it needed no change either — it simply stops mattering for an id that can't reach `ColonyPool` in the first place.
- **Water is both a utility *and* packageable cargo** (the actual decision, resolving #380's own "does losing shippability matter" question): a `water_bottling_plant` runs a switchable pair of recipes, `bottle_water` (banked water → `water_container` commodity) and `unbottle_water` (the reverse), so a colony can still export a genuine surplus by convoy — crated, exactly as §5A's crating-via-recipe decision above describes, the same route `power` would need for charged-cell export.

**The founding water seed survives via a free `water_tank`.** Every starter supply package still seeds water (`content/base/supplies.yaml`), but a resource-classified deposit lands in `ColonyResourcePool`, which is wiped to nothing at the end of the first sol unless something banks it — so the founding kit now always includes `water_tank` (`starter_kit: true`, `slot_cost: 0`, `storage: [{id: water, quantity: 150.0}]`), free and automatic, not part of the player-choosable loadout (the wizard's kit preview is read-only since issue #312). The loader's supply-package validation (`content/loader.rs`) was widened to accept resource ids as well as commodities — previously restricted to "you can't load `power` onto a lander," which is still the default assumption for most resources, but no longer a blanket rule now that a resource can have starter-kit storage to land in.

Proven end-to-end by `outpost_core`'s `found_colony_at_site_seeds_a_resource_from_supply_and_it_survives_via_the_free_tank` (the full seeded amount is available immediately at founding, then capped at the tank's capacity after the first sol rather than evaporating outright), `turn::tests::water_declared_as_a_resource_cannot_be_traded_even_with_a_route_and_stock`, and `outpost_web`'s `water_bottling_plant_crates_and_uncrates_water_from_real_pack` (the crating round trip against the real content pack).

**Waste overflow: hex contamination.** When waste outruns processing and storage, it contaminates the colony's hex: habitability drops and deposit yields fall. Chosen over a stability hit, back-pressure on producers, or population health effects, because it is the only genuinely *new* kind of consequence — the others reuse levers brownout and labour shortage already pull, or land on stability, an increasingly undifferentiated "things are bad" number. It also makes the 100 km hex map (§8.1) matter beyond site selection: neglect can foul the ground a colony lives on. **Contamination is remediable** — the right technology lets a player clean up a contaminated hex and remove the penalty, so it is a lasting consequence rather than a permanent one. Needs per-hex contamination state, a tech-gated cleanup action, and UI showing affected hexes and severity.

**Issue #387 implements the contamination state, its trigger, and its two consequences; issue #388 implements remediation** (the two together are contamination's full up/down lifecycle). `HexCell.contamination: f32` (`map.rs`) is a new per-hex field in `[0.0, 1.0]`, raised by `PlanetMap::contaminate` and never decremented by anything in this pass. The trigger is evaluated once per colony per sol, immediately after that sol's production step (`lib.rs`'s "Step 3a1"): `overflow = colony.resources.amount("waste") − amount("waste_processing") − waste_bunker capacity`; a positive overflow converts to a contamination increment via `PlanetMap::contamination_increment_for_overflow`, which scales linearly from `0.0` up to a per-sol ceiling (`CONTAMINATION_MAX_INCREMENT_PER_SOL = 0.1`) at a reference overflow of one bunker's worth (`CONTAMINATION_OVERFLOW_SCALE_KG = 40.0`) and holds flat beyond it — so a single very bad sol cannot instantly max out a hex, but sustained overflow across many sols still saturates it. Deliberately runs *before* the next sol's `bank_and_clear` would otherwise silently discard the same excess — contamination observes the overflow that step is about to evaporate, rather than changing what evaporation itself does.

Both consequences apply as independent multiplicative factors **at the point of use**, not baked into any stored per-body or per-colony field — contamination is a per-hex, per-colony effect that changes sol to sol, so folding it into `Body::habitability` (a slower-changing, body-wide, mitigation-driven score) or permanently into `colony.habitability_modifier` would double-count it against remediation and against any future body-level habitability change. Deposit yield: `colony_deposit_richness` scales the whole merged per-commodity figure — hex-specific deposits and the body-level abundance flavor (#232) merged in alongside them via `.max()` — by `(1.0 − contamination)` before it reaches `compute_deposit_ratio`'s gating. Scaling only the hex-sourced entries was tried first and found to be a real gap in review: since the merge is a `.max()`, an untouched, higher body-level abundance figure for the same commodity would silently win and erase the contamination penalty entirely — the colony is extracting from this specific fouled hex regardless of which source produced the larger number, so the final merged value has to reflect that, not just one input to it (`tests::contamination_scales_the_merged_richness_even_when_body_abundance_would_otherwise_mask_it` locks this in). Habitability: `colony::contamination_habitability_factor(contamination)` — linear from `1.0` (pristine) down to a floor of `0.5` (maximally contaminated, never zero) — multiplies `colony.habitability_modifier` at the one call site that feeds `process_production_scaled`'s `productivity_multiplier`.

No fog-of-war interaction was needed: despite §9C's design intent, no per-hex visibility gating exists in the engine today (confirmed by inspection — `Body.surveyed` is a system-scale UI marker only, per `#235`'s note in §9A), so contamination state needs no fog-aware gating to add later without rework.

Wired through to both hosts' `PlanetHexWire`/`DepositWire` (issue #220/#307-era duplication — `outpost_web`'s `query_routes.rs` and `outpost_tauri`'s `commands.rs` each own a hand-kept copy) and rendered in `PlanetHexMap.vue` as a translucent yellow-green overlay blended over every other terrain/water/vegetation/elevation/temperature layer, plus a tooltip readout and hover warning — following the same optional-field, graceful-fallback pattern `water_coverage`/`vegetation_density` established in #316.

Proven by `outpost_core`'s `map::tests` (`contaminate_raises_and_clamps_at_one`, `contaminate_zero_or_negative_delta_is_a_no_op`, `contaminate_missing_coord_does_not_panic`, `contamination_increment_scales_with_overflow_and_caps_per_sol`), `colony::production::tests`' `contamination_habitability_factor_*` suite, `outpost_core::tests::waste_overflow_contaminates_the_colonys_own_hex` (end-to-end via a real `AdvanceColonySol`, proving overflow triggers contamination and that it accumulates across sols), and `frontend`'s `PlanetHexMap.test.ts` contamination-overlay suite.

**Issue #388 implements remediation as a construction-style project, the same shape #306's site-preparation projects use, rather than an instant free action or a recurring upkeep toggle** — surfaced as an explicit design decision, since #388's own text left it open. `BuildingDef.contamination_reduction: f32` (`content/types.rs`) mirrors `grants_slot_capacity`'s "non-zero makes this a project, not a building" shape exactly: a queued project with `contamination_reduction > 0.0` completes into a hex contamination reduction (`PlanetMap::remediate`, `HexCell.contamination` floored at `0.0`, never negative) instead of becoming a `PlacedBuilding`, costing labour and materials and taking turns like any other construction project — the same real tradeoff (a build slot and resources) that made neglecting waste a genuine cost in the first place. A new `BuildingCategory::Remediation` keeps this distinct from `Infrastructure`'s existing "project completes into `grants_slot_capacity`" invariant (enforced by its own content-loader test) rather than overloading that category with a second, incompatible completion shape.

The turn-pipeline completion branch (`lib.rs`'s `Command::AdvanceColonySol` handler) snapshots `contamination_reduction`-bearing building types the same way `slot_grants` already snapshots `grants_slot_capacity` ones, then — since resolving a colony's hex needs a mutable borrow of `self.state.planet_maps` that can't overlap the colony loop's mutable borrow of `self.state.colonies` — collects completed remediation projects into a `Vec` and applies them in a second pass afterward, mirroring #387's own overflow-detection two-pass shape. `Event::ContaminationRemediated` fires on completion (mirroring `Event::SlotCapacityExpanded`'s "instead of `BuildingConstructed`" contract) — absent for a colony with no hex placement, since the project's cost was still paid but there's nowhere to apply the reduction, matching how deposit depletion silently skips an unplaced colony too.

Tech-gated behind a new `bioremediation` tech node (`content/base/tech.yaml`, tier 2, life_sciences, prerequisite `hydroponics`) via the existing generic `BuildingDef.tech_prerequisite` enforcement in `Command::QueueConstruction` — no new gating code needed, since that check already applies uniformly to every building/project type. The authored `hex_remediation` project (`content/base/buildings.yaml`) removes `0.3` contamination severity per completion for `25` structural metal and `2` labour over `3` turns; a hex above `0.3` severity needs more than one remediation cycle.

Proven by `outpost_core`'s `map::tests` (`remediate_lowers_and_floors_at_zero`, `remediate_zero_or_negative_delta_is_a_no_op`, `remediate_missing_coord_does_not_panic`), `outpost_core::tests::a_completed_remediation_project_lowers_contamination_instead_of_placing_a_building` and `remediation_floors_contamination_at_zero_rather_than_going_negative` (synthetic-registry, mirroring #306's site-prep completion tests), and `outpost_web`'s `hex_remediation_requires_bioremediation_and_lowers_contamination_from_real_pack` (real content pack: `TechLocked` before `bioremediation` is researched, then a full queue-advance-complete cycle against the real `hex_remediation` project once it is).

**Storage is a building attribute, not a third `ResourceKind`.** `ResourceKind` stays `Flow`/`Capacity` — no `Stock` variant. Whether a resource carries over is a property of what the player has *built*, not of how the resource is authored: `BuildingDef.storage: Vec<Ingredient>` (same shape as the existing `maintenance` field) lists per-resource banking capacity, summed across every completed instance of that building a colony has (`colony::storage_capacities`). `ColonyResourcePool::bank_and_clear` replaces the unconditional `clear()` in the turn pipeline (§5's "Colony resources do not persist across sols" is now the *zero-storage* case, not the only case): each resource is capped at its colony's built capacity for that id — `min(amount, capacity)` — rather than dropped to zero, so banked stock survives into the next sol up to what was actually built, and everything above capacity (or with no storage at all) still evaporates exactly as before. Storage counts a building regardless of staffing or `paused` state — a battery holds its charge whether or not its crew is on shift.

**Crating for convoy is a real commodity, made by a recipe.** Converting a banked utility into something a convoy can carry (charged cells, water containers) goes through an ordinary recipe that consumes the utility and outputs a genuine tradeable commodity — reusing the existing commodity/trade machinery unchanged, rather than teaching convoys a second resource vocabulary.

**Fixed links carry capacity and transmission loss**, not a boolean connection — consistent with §8.1's infrastructure edges already modelling throughput. A powerline has a throughput and a loss percentage, so upgrading the line is a real choice against living with the loss.

**Maintenance keeps both an eventual failure model** — a condition stat that degrades under unmet maintenance and a per-sol breakdown roll whose odds worsen as condition drops — **but it is not built here.** No random building-failure system exists anywhere in the engine today; today's maintenance shortfall only scales output down (#180), and adding both halves of a real failure model is the largest single piece of the utilities work by itself. Filed as its own follow-up issue rather than bundled into this pass.

**`power` keeps both of its existing meanings unchanged.** The per-capita household draw (a `NeedDef` reading `ColonyResourcePool`) and a building's `power_delta` industrial capacity/demand ratio (`colony::production::compute_power_grid_scaled`, which never touches the pool) answer different questions — how much power colonists need to live, versus how much the grid can supply before recipes throttle — and unifying the utility *storage* model does not require collapsing that distinction.

**What's implemented now.** The generic mechanism — `BuildingDef.storage`, `colony::storage_capacities`, `ColonyResourcePool::bank_and_clear` wired into the turn pipeline in place of the unconditional `clear()` — plus three worked examples: `battery_bank` (`content/base/buildings.yaml`), a passive storage building banking 40 MW-equivalent of `power`, the full water migration described above (`water_tank` storage, `water_bottling_plant` crating), and `waste` itself: a per-sol flow resource produced as a byproduct of refining recipes (`refine_ore_to_plate`, `smelt_conductive_ore` each now output 1 kg of `waste` alongside their main product), banked by a `waste_bunker` (40 kg capacity, same shape as `battery_bank`/`water_tank`), a `waste_processing` capacity resource re-established each sol by a `recycling_plant`'s `process_waste` recipe (authored identically to `housing`, a standing capacity rather than a stock), overflow that contaminates the colony's hex (#387) with tech-gated remediation to clean it back up (#388), and — issue #389 — a `waste_compactor` crating waste into shippable `waste_container` cargo via a switchable `crate_waste`/`uncrate_waste` pair, the exact mirror of `water_bottling_plant`'s `bottle_water`/`unbottle_water`: a colony with spare `waste_processing` capacity can genuinely absorb another colony's surplus by convoy, closing out #381's own taxonomy entry for waste ("convoy, to a colony with spare capacity"). Proven by `outpost_core`'s `power_surplus_evaporates_every_sol_without_a_storage_building` (regression: unchanged pre-#348 behaviour with no storage building present) and `battery_bank_carries_surplus_power_across_sols_up_to_its_capacity` (surplus survives into the next sol up to capacity, and still evaporates above it), the water-specific tests listed above, `outpost_web`'s `refinery_recipes_emit_waste_byproduct_from_real_pack`, `waste_bunker_carries_waste_across_sols_up_to_its_capacity`, `recycling_plant_reestablishes_waste_processing_capacity_each_sol`, and — for #389 — `outpost_web`'s `waste_compactor_crates_and_uncrates_waste_from_real_pack`, mirroring the water-bottling test's structure exactly.

With #386-#389 all landed, #381's own four-part split is complete — waste now has a full lifecycle: produced as a byproduct, banked, contaminates its hex on overflow, is remediable, and can be crated for convoy shipment.

**Fixed-link capacity and transmission loss are implemented (#383).** `map::InfraType` gains a `Powerline` variant (base throughput 100 kW, 12% transmission loss, cost factor 2.0 — balance dials, expect retuning), and `map::InfraEdge` carries a `loss_pct: f32` field populated from the built edge's type (`InfraType::base_loss_pct`); Road/Rail/Pipeline carry `0.0` for now since cargo trade over `TradeRoute` has no loss concept yet (capacity only) — wiring cargo loss is future work, not blocked by this. A powerline does **not** wire up a `TradeRoute` on `BuildInfrastructure` (`InfraType::carries_cargo` gates that, `Powerline` is the one `false` case) — it moves the `power` utility instead. Each sol, before any colony's production step runs, `colony::resolve_power_transfers` walks every `Powerline` edge across every body's `PlanetMap`, moving surplus capacity from a colony whose grid has `capacity > demand` toward a connected colony with a deficit, net of the edge's `loss_pct` and capped by its `throughput` — the resulting net import/export folds into `process_production_scaled`'s new `power_import` parameter, so a surplus neighbor's power arrives before that sol's brownout ratio is computed. `Event::InfrastructureBuilt`/`InfrastructureDemolished`'s `route_id` is now `Option<Uuid>` to reflect that a powerline's build/demolish has no trade route to report. Frontend: `PlanetView.vue`'s infra-type picker gains a Powerline option and the edge list shows loss %; `PlanetHexMap.vue` renders powerlines in a distinct color and adds a hover tooltip (`<title>`) summarizing type/throughput/loss on every edge. The network link for #382 (networking/compute) is expected to reuse this same model rather than inventing a second one.

**What's filed as follow-up issues, per this section's own decisions above:** networking/compute as a utility feeding population morale (#382); the maintenance condition-stat-plus-breakdown-roll failure model (#384).
...
- **Slider surface is Tauri-only**, matching where §8.3AA's sliders already live (`MainMenuView.vue`) — `outpost_web`'s browser-mode `/new-game` view is not widened; its `handle_new_game` always resolves the nine new fields to `SystemGenParams::default()`.
...
### 8.3G Direct-founding transit delay (issue #359 remainder)

`content/base/colonization.yaml`'s `outpost`/`colony` cost profiles (charging model) landed first; this closes the issue's remaining "launched expedition" half — a directly-founded colony now arrives after a distance-derived delay rather than materialising the instant it is paid for.

**Mechanics.** `Command::FoundColonyAtSite` with a `sponsor_colony_id` still validates and bills the sponsor *immediately* — an unaffordable founding still changes nothing — but the resulting `colony::Colony` is no longer created in that same command. Instead a `PendingColony` record (name, resolved site/coord, resolved starter supplies, home-body-link data, sponsor id) is pushed onto `GameState.pending_colonies` with `sols_remaining` set from a new `GameEngine::body_transit_sols(from_body, to_body)` — the same distance-derived reading `route_transit_sols` already uses for trade convoys (a body separation read as **sols, not months**; same-body or unknown-origin foundings still take `trade::DEFAULT_TRANSIT_SOLS`, so even a same-body founding is a beat of transit, not instant, matching the "surface convoy" framing below). Each `AdvanceColonySol` ticks every pending record down (`PendingColony::tick`, mirroring `OrbitalConstructionProject::tick`'s countdown-then-materialise shape) and, at zero, calls the same `materialize_founded_colony` helper the bootstrap (unsponsored) path calls directly — so a sponsored and an unsponsored founding produce byte-for-byte the same `ColonyFoundedAtSite`/`ColonyPlacedOnMap`/starter-kit/home-body-link events, just on different schedules. `Event::ColonyFoundingLaunched` fires at billing time so a host can show "in transit" immediately; the arrival events fire later, unprompted, from whichever `AdvanceColonySol` call crosses zero.

The target site is reserved for the *entire* transit even though no colony or map node exists yet to occupy it: the occupancy check in `FoundColonyAtSite` scans `GameState.pending_colonies` for a matching `(body_id, coord)` in addition to the map's own `colonies` list, so a second founding can't land on the same hex while the first is still in flight.

**Bootstrap exception unchanged.** `sponsor_colony_id: None` — the game's first colony, arriving from off-system with nobody to bill — still materialises in the same command, exactly as before #359. This is what every pre-existing engine test and the balance harness's `ColonyConfig` rely on, and it's a deliberate exception (see the original #359/#311 "no sponsor to bill" resolution), not an oversight.

**Design calls made for this remainder** (per the issue's own open-question list):

- **Colonists in transit: still removed at launch**, not held in limbo and returned on failure. Unchanged from the charging-model stage — simplest, and consistent with how commodities are charged (withdrawn at dispatch, not refundable). Revisit if it reads wrong once transit is visible in play (the issue's own framing).
- **Expedition failure/abort: not modelled.** A launched founding always arrives; there is no hazard roll or cancellation path, unlike `expedition.rs`'s survey missions. Trade convoys (`trade.rs`) are the closer precedent — dispatched cargo always arrives — and a founding is architecturally closer to a convoy (withdraw-then-credit) than to a survey (which can anomaly/fail). No engine precedent forced an answer either way; convoy-like "always arrives" was chosen to keep the mechanic legible on its first pass. A future issue can add risk once outpost-vs-direct-founding is played and a "founding can fail" scenario earns its complexity.
- **Vehicle framing / orbital-prerequisite gating: declined, single distance-derived delay is sufficient.** The issue asked whether same-body founding (a "surface convoy") and cross-body founding (a "colony ship") need two distinct transit calculations/gates, and whether cross-body founding should additionally require an orbital capability (station, launch facility, tech) before it can even be attempted. Decision: **no** to both, for now. `body_transit_sols` already produces a materially different number for same-body vs. cross-body (the `DEFAULT_TRANSIT_SOLS` floor vs. an actual `compute_travel_time` reading) without needing two code paths, so the "vehicle" distinction is flavor text over one formula, not a second mechanic — adding a real orbital prerequisite on top would gate an already-expensive `colony` profile behind a second resource sink with no playtesting signal that founding is otherwise too easy (the same "don't invent a threshold with no data" reasoning §8.3D used for promotion prerequisites). If actual play shows cross-body founding needs a harder gate than cost + distance, add the orbital prerequisite then, keyed off `system::HARSH_WORLD_CAPABILITY_ID`-style unlock rather than reworking the transit formula.

**Balance-harness coverage note.** `outpost_harness`'s `ColonyConfig`/`simulate.rs` performs static commodity-chain closure analysis (production vs. consumption, per commodity) and has no notion of issuing `Command`s or advancing sols at all — the same structural limitation §8.3D's coverage note hit for the outpost range gate. There is no natural harness bundle for "both routes total the same price and the direct route arrives after N sols"; that is exactly what the engine tests below assert instead. A future harness extension that can drive `Command` sequences (not just static chains) would be the right place to add this, rather than forcing a schedule/command-sequence assertion into a tool built for steady-state production math.

**Test coverage** (`outpost_core/src/lib.rs`): `sponsored_founding_launches_but_does_not_materialize_immediately` (billed at once, colony does not exist yet, `ColonyFoundingLaunched` reports a non-zero `sols_remaining`), `pending_colony_materializes_after_its_transit_delay` (advancing exactly `sols_remaining` sols produces `ColonyFoundedAtSite`/`ColonyPlacedOnMap`, the colony exists with the right population/home body, the pending record is consumed), `in_flight_founding_reserves_its_site_against_a_second_founding` (a second founding at the same site while the first is still in flight is rejected with `SiteOccupied`). `outpost_core/src/snapshot.rs::round_trip_pending_colonies` confirms `GameState.pending_colonies` survives a save/load cycle (schema version bumped to 13).
