...
Both consequences apply as independent multiplicative factors **at the point of use**, not baked into any stored per-body or per-colony field — contamination is a per-hex, per-colony effect that changes sol to sol, so folding it into `Body::habitability` (a slower-changing, body-wide, mitigation-driven score) or permanently into `colony.habitability_modifier` would double-count it against remediation and against any future body-level habitability change. Deposit yield: `colony_deposit_richness` scales a hex's own deposit entries by `(1.0 − contamination)` before they reach `compute_deposit_ratio`'s gating — the body-level abundance figure merged in alongside it (comparative flavor, #232) is untouched. Habitability: `colony::contamination_habitability_factor(contamination)` — linear from `1.0` (pristine) down to a floor of `0.5` (maximally contaminated, never zero) — multiplies `colony.habitability_modifier` at the one call site that feeds `process_production_scaled`'s `productivity_multiplier`.
