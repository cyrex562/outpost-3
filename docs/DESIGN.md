...
**Still open (issue #272, gap 5):** whether one slot for three functions is *well-balanced* against keeping the standalone buildings separate. `colony_hq`'s rates were set to exactly match the sum of what it replaces, which is a defensible starting point and nothing more. That needs playtest data, not more analysis, so no numbers have been changed on speculation.
