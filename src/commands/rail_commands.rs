...
impl Command for BuildRail {
    ...
    fn execute(&self) -> Result<Vec<EventType>, Self::Error> {
        ...
        info!(
            ...
            from_q = self.from_tile.q,
            from_r = self.from_tile.r,
            to_q = self.to_tile.q,
            to_r = self.to_tile.r,
            rail_type = ?self.rail_type,
            cost_credits = cost.get(ResourceType::Credits),
            cost_steel = cost.get(ResourceType::Steel),
        ...
        match self.upgrade_type {
            UpgradeType::UpgradeType(new_type) => {
                ...
                info!(
                    colony_id = ?self.colony_id,
                    segment_id = ?self.segment_id,
                    old_type = ?self.current_rail_type,
                    new_type = ?new_type,
                    cost_credits = cost.get(ResourceType::Credits),
                    "Rail type upgraded"
                );
            ...
            UpgradeType::AddTrack => {
                ...
                info!(
                    colony_id = ?self.colony_id,
                    segment_id = ?self.segment_id,
                    old_count = self.current_track_count,
                    new_count = new_count,
                    cost_credits = cost.get(ResourceType::Credits),
                    "Rail track added"
                );
...
impl Command for BuildStation {
    ...
    fn execute(&self) -> Result<Vec<EventType>, Self::Error> {
        ...
        info!(
            ...
            connected_segments = self.connected_segments.len(),
            cost_credits = cost.get(ResourceType::Credits),
            "Station built"
        );
...
impl Command for PurchaseCar {
    ...
    fn execute(&self) -> Result<Vec<EventType>, Self::Error> {
        ...
        info!(
            ...
            cost = self.car_type.cost_credits,
            "Freight car purchased"
        );
