...
impl GameEngine {
    ...
    pub fn apply(&mut self, cmd: &Command) -> Result<Vec<Event>, EngineError> {
        match cmd {
            Command::AdvanceColonySol => {
                ...
                // can extrapolate trajectory via linear trend analysis.
                for (colony, pop) in self
                    .state
                    .colonies
                    .iter()
    ...
    /// Advance the simulation up to `n` colony-sol turns, halting at the first
    /// interrupt whose tier is >= `threshold`.
    ///
    /// Returns [`AdvanceResult::Halted`] on the first qualifying interrupt,
    /// or [`AdvanceResult::Completed`] for a clean run. Both carry a digest of
    /// `Notable` and `Ambient` interrupts accumulated during the run.
    ///
    /// # Errors
    ///
    /// Propagates any [`EngineError`] from individual `AdvanceColonySol` turns.
    pub fn advance_until_interrupted(
        ...
        for _ in 0..n {
            self.apply(&Command::AdvanceColonySol)?;

            let turn_interrupts = self.collect_turn_interrupts();

            for interrupt in turn_interrupts {
                if interrupt.tier >= threshold {
                ...
                // Below threshold: accumulate Notable/Ambient in digest.
                digest.push(interrupt);
            }
        }
    ...
    /// Collect interrupts generated by the current game state after a sol turn.
    ///
    /// Emits `Urgent` predictive warnings when the stability trajectory projects
    /// a crash within `PREDICTIVE_WARNING_ETA` turns.
    fn collect_turn_interrupts(&self) -> Vec<Interrupt> {
                ...
                if let Some(eta) = tracker.eta_to_floor(STABILITY_CRISIS_FLOOR) {
                    if eta <= PREDICTIVE_WARNING_ETA {
                        interrupts.push(Interrupt::new(
                            ...
                            Some(colony.id),
                            format!(
                                "Colony '{}': stability declining — crisis in ~{eta} turns (current: {:.2})",
...
mod tests {
    ...
    // ── Interrupt tiers + advance_until_interrupted (Done-when tests for issue #23) ──

    /// Done-when: advance halts on first Urgent interrupt when threshold = Urgent.
    #[test]
    fn advance_halts_on_urgent_interrupt() {
        use crate::interrupt::{AdvanceResult, InterruptSource, Tier};

        let mut engine = GameEngine::with_seed(42);
        // No colony = no interrupt; add a colony with steep declining stability.
        let events = engine
            .apply(&Command::FoundColony {
        ...
        // Pre-load tracker with a steep declining trajectory.
        ...
        let result = engine
            .advance_until_interrupted(20, Tier::Urgent)
    ...
    /// Done-when: clean advance to N turns returns Completed.
    #[test]
    fn advance_completes_n_turns_without_interrupts() {
        ...
        let result = engine
            .advance_until_interrupted(5, Tier::Urgent)
            .expect("advance should not error");
        assert!(
    ...
    /// Done-when: predictive warning fires *before* the actual crisis.
    #[test]
    fn predictive_warning_fires_before_crisis() {
        ...
        // Steep declining trajectory — will project crash well within 10 turns.
        let tracker = engine.state.stability_trackers.entry(colony_id).or_default();
        ...
        match result {
            AdvanceResult::Halted { interrupt, .. } => {
                // Warning fired: stability at time of warning is still above floor.
                if let InterruptSource::PredictiveWarning { quantity, .. } = interrupt.source {
                    assert!(
                        quantity > 0.2,
                        "predictive warning should fire before stability hits crisis floor (0.2); was {quantity}"
                    );
                }
            }
            AdvanceResult::Completed { .. } => {
                panic!("expected Halted but got Completed");
            }
        }
    }

    /// Done-when: Blocking threshold lets Urgent interrupts pass into the digest.
    #[test]
    fn urgent_interrupt_accumulates_in_digest_when_threshold_is_blocking() {
        ...
        // Steep declining stability — would normally fire Urgent warning.
        let tracker = engine.state.stability_trackers.entry(colony_id).or_default();
        ...
        assert!(
            !digest.is_empty(),
            "digest should contain accumulated interrupts; got empty digest"
        );
    }
